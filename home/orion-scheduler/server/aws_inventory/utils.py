...
def get_aws_regions(aws_access_key_id, aws_secret_access_key, session=None):
    ...
    for region in region_list:
        ...
        if (region['OptInStatus'] != 'not-opted-in'):
            ...
            pass
        if (region['OptInStatus'] == 'not-opted-in'):
            logger.info('region not opted: {}'.format(region))
            print('region not opted: {}'.format(region))

    return regions["Regions"]


def get_not_opted_regions(aws_access_key_id, aws_secret_access_key, session=None):
    not_opted_regions = []
    # We get the regions list through EC2.
    if not session:
        session = boto3.Session(aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
    ...
    for region in region_list:
        ...
        if (region['OptInStatus'] == 'not-opted-in'):
            logger.info('region not opted: {}'.format(region))
            print('region not opted: {}'.format(region))
            not_opted_regions.append(region['RegionName'])

    return not_opted_regions
